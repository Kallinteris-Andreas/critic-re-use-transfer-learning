...
class model():
    def __init__(self, num_actions_spaces: list[int], num_states_spaces: list[int], num_state_global: int, min_action: float, max_action: float, config: dict, torch_device="cpu"):
        ...
        self.twin_critic = modules.twin_critic(sum(num_actions_spaces), num_state_global, config['TD3']['q_bias'], device=self.device)
        self.target_twin_critic = copy.deepcopy(self.twin_critic)
        self.twin_critic_optimizer = torch.optim.Adam(self.twin_critic.parameters(), config['TD3']['optimizer_gamma'])
    ...
    def train_model_step(self, zoo_env) -> None:
        ...
        with torch.no_grad():
            ...
            # compute y
            qt0, qt1 = self.target_twin_critic(new_state_batch, target_actions_batch)
            y = reward_batch + ~terminal_batch * self.discount_rate * torch.min(qt0, qt1)

        # compute critic losses
        q0, q1 = self.twin_critic(old_state_batch, actions_batch)
        ...
        if (self.total_step_iterations % self.policy_update_frequency) == 0:
            for agent_id in range(self.num_agents):
                ...
                policy_loss = (-self.twin_critic.critic_0(old_state_batch, team_actions)).mean()
